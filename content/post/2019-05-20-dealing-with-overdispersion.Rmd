...
You could extract the variance of the parameter estimated, multiply it by \u03D5:

```{r scalevar}

scalederr <- log(summary(glm1)$coefficients[,2]) * phi

old_CI <- confint.default(glm1, type="wald")

lci <- exp(log(coef(glm1)) - (1.96 * scalederr))
uci <- exp(log(coef(glm1)) + (1.96 * scalederr))
